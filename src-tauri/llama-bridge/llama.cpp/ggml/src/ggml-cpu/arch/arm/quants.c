...
#undef STQ1_0_DOT_HALF
    ...
    *s = sumf;
#else
    // The optimized STQ implementation above uses AArch64 table-lookup,
    // zip, and horizontal-reduction intrinsics. ARMv7 NEON does not expose
    // those operations, so the 32-bit compatibility ABI uses the exact
    // generic kernel rather than failing the universal Android build.
    UNUSED(x);
    UNUSED(y);
    UNUSED(nb);
