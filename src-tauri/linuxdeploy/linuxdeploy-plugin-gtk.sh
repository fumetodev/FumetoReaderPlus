...
# Vendored copy of linuxdeploy-plugin-gtk from
# https://github.com/tauri-apps/linuxdeploy-plugin-gtk (MIT licensed),
# upstream commit b5eb8d05b4c0ed40107fe2158c5d8527f94568ef of 2024-01-04.
#
# One deliberate change from upstream, in the AppRun hook this script writes
# into the AppDir: GDK_BACKEND is defaulted to x11 instead of being forced to
# it, so a user who launches the AppImage with GDK_BACKEND=wayland keeps that
# choice. Everything else is byte-identical to the upstream revision above.
# README.md next to this file explains how the bundler picks it up.
...
export GDK_BACKEND="${GDK_BACKEND:-x11}" # x11 unless the user chose a backend; the Wayland backend crashes on some setups: https://github.com/tauri-apps/tauri/issues/8541
