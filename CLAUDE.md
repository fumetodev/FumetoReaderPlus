...
- **No unique-direction IndexedDB cursors.** WebKit's IndexedDB (the Linux WebView) rejects `nextunique`/`prevunique` cursors on any index with "Unable to open cursor", even on an empty store, so Dexie's `uniqueKeys()` is banned; use `distinctKeys()` from `src/lib/db/unique-keys.ts`, which scans keys and drops consecutive equals with the engine's own `indexedDB.cmp`. Chromium never surfaced this, so an Android run proves nothing here.
