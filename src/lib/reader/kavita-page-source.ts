/**
 * Kavita page source — fetches page images from a Kavita server.
 *
 * Similar to KomgaPageSource but simpler: Kavita pages are 0-indexed
 * (no conversion needed) and identified by chapterId + page index.
 */
...
import type { KavitaServerClient } from '$lib/kavita/kavita-server-client.js';

/** Maximum number of pages to keep in the in-memory cache */
const MAX_CACHED_PAGES = 20;

export class KavitaPageSource implements PageSource {
	private client: KavitaServerClient;
	...
	constructor(client: KavitaServerClient, chapterId: number, pageCount: number) {
		this.client = client;
	...
	async getPage(index: number, options: PageRequestOptions = {}): Promise<Blob> {
		...
		// Fetch from server (Kavita is 0-indexed, no conversion needed)
		const blob = await this.client.fetchPage(this.chapterId, index, options.signal);
		throwIfPageRequestCancelled(options.signal);
